<?php
	...
	if(isset($_GET['getProductionFromExcel'])){
		...
		for ($row=2;$row<10000;$row++){
			...
			$vespaleta=$excFile->getActiveSheet()->getCellByColumnAndRow(19,$row)->getCalculatedValue();
		$jsonXLSX.=$delim.' "'.$code.'":{"role":"'.$role.'", "customer":"'.$customer.'", "fullName":"'.$fullName.'", "form":"'.$form.'", "code":"'.$code.'", "shortName":"'.$shortName.'", "color":"'.$color.'", "totalUnits":"'.$totalUnits.'", "boxing":"'.$boxing.'", "layers":"'.$layers.'", "h":"'.$h.'", "target":"'.$target.'", "proc":"'.$proc.'", "sap":"'.$SAP.'", "gost":"'.$GOST.'", "sto":"'.$STO.'","vespaleta":"'.$vespaleta.'"}';
			$delim=",";
		}
		$jsonXLSX.="}}";
		echo $jsonXLSX;
