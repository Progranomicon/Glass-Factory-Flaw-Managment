...
function log(text){
	el('log').innerHTML=text+'<br><br>'+el('log').innerHTML;
}
...
function error_handler(){
	//alert('Проблемы с AJAX.');
}
function getMessage(sender, message){
	//alert(jstr(message));
	if(message.result.status!="ok") alert(sender+": "+message.result.message);
}
var timer_on=true;
...
function validateFloatInput(str){//заменяет запятые точками
	var pos;
	var result=str;
		if(str.indexOf(',', 0)>=0){
			pos=str.indexOf(',', 0);
