<?php 
	...
	if (isset($_GET['get_defects'])){
		...
		echo '{';
		while($row=mysql_fetch_assoc($res)){
			echo '"'.$row['id'].'":{"id":"'.$row['id'].'", "date_start":"'.$row['date_time_start'].'", "date_end":"'.$row['date_time_end'].'", "share":"'.$row['flaw_part'].'", "action":"'.$row['corrective_action'].'", "form":"'.$row['form_number'].'", "type":"'.$row['defect'].'"}, ';
		}
