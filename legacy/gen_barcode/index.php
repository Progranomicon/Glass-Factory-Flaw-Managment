...
//кэшируем всю продукцию в массив
    ...
    print '<script type="text/javascript">';
			echo ' var Production=[];
		'; 
        $RProduction = mysql_query("SELECT * FROM productionutf8 WHERE IsDeleted!=TRUE");
            while ($RetProd = mysql_fetch_assoc($RProduction)) {
                echo 'Production.push([' . $RetProd['id'] . ', "' . $RetProd['type'] . '", "' .$RetProd['glass_color']. '", "' . $RetProd['format_name'] . '", "'.$RetProd['units_number'].'", "' . $RetProd['pallet_size'] . '", "' . $RetProd['boxing'] . '", "' . $RetProd['MorePics'].'"]);';
...
// вывод макета этикетки
    ...
    print ' <div id="label_div1">
                <div id="label_div_left1">
                    <img SRC="/img/rst_logo3.png" style="padding-top:10px;"><br>
					<img src="/../barcode.php?print=1&code=BarCodeHere&scale=2&mode=png&encoding=128&random=50027175" alt="Barcode-Result" style="-moz-transform : rotate(-90deg); margin-top:2cm; margin-left:-2cm;"/>
                                        
                </div>
                <div id="label_div_right1"><br>
                    <img SRC="/img/4_logo2.png"><br>
                    <span id="MorePics"></span>
                    <br>
                ...
                <div id="label_div_center1">
                    ...
                    ГОСТ Р ИСО 9001-2008, ГОСТ Р ИСО 22000-2007, <br>ГОСТ Р ИСО 14001-2007 <br>(Сертификат соотвествия №СДС.Э.СМК 000850-12 от 30.11.2012)<br>
                    ...
                    <p><FONT size=3 ><b><span id="product_s"><A HREF="javascript:show_input();">Выбрать продукцию</A></span></b></FONT></p>
                    <span style="font-size:0.75em;">СТО 99982965-001-2008</span><br>
...
{/* генерация этикеток*/
     ...
     for($iteration=$_POST['pallet_sh'];$iteration<($_POST['pallet_sh']+$_POST['count_of_labels']);$iteration++){
        ...
        if ($frmt['MorePics']!=1)
        {     
            print '<img SRC="/img/rst_logo3.png">';
        }
        else echo '<img SRC="/img/rsz_logo.png">';
        echo '    <br>
				<img src="/../barcode.php?print=1&code='.(date("dmy")).($_POST['line_number']).((sprintf ("%03d",$_POST['id_s'])).(sprintf ("%04d",$iteration))).'&scale=2&mode=png&encoding=128&random=50027175" alt="Barcode-Result" style="-moz-transform : rotate(-90deg); margin-top:1.8cm; margin-left:-1cm;"/><br>';
         if ($frmt['MorePics']==1)
             {    
                                echo '<img SRC="/img/MorePicsCir.png" style="margin-top:50px;"><img SRC="/img/EAC.JPG" style="margin-top:50px;">';;
             }
         /*if ($_POST['id_s']==0 || $_POST['id_s']==134)
             {    
        ...
        print '</div>
         ...
         if ($frmt['MorePics']==1)
             {
                echo '<img SRC="/img/MorePicsTri.png"><br>';
             }
         echo'           <br>
        ...
        if (strlen($frmt['format_name'])>30) echo '4';
        else echo '5';
        echo '"  ><b><i><u>'.$frmt['format_name'].'</u></i></b></FONT><br>';
        if ($_POST['id_s']==0 || $_POST['id_s']==134)
            echo '<span style="font-size:0.75em;">ГОСТ Р 52327-2005';
        else
            echo '<span style="font-size:0.75em;">СТО 99982965-001-2008';
        if ($_POST['id_s']==107 || $_POST['id_s']==110)
            echo ', SAP 40009335';
        if ($_POST['id_s']==127 || $_POST['id_s']==128)
            echo ', SAP 40006122';
        if ($_POST['id_s']==0 )
            echo ', код продукции 00000000001*';
        if ($_POST['id_s']==134 )
            echo ', код продукции 30301000008';
        echo'</span></div>';		
        if ($_POST['id_s']==88 || $_POST['id_s']==107 || $_POST['id_s']==110 || $_POST['id_s']==127 || $_POST['id_s']==128)
             {
                echo '<div style="margin-left:-20px;" >Срок годности - до '.date('d.m.').(date("Y")+1).' г.<br></div>';
             }
                 
        echo '('.$frmt['boxing'].')<br>
			     <br>
        <div style="text-align:left;text-indent:0px; margin-left:80px;">
        ...
        ';
		$file=fopen("Nomera.postanovok","r");
		while (!feof($file)) {
			$buffer = fgets($file);
			$els=explode(" ", $buffer);
			if ($_POST['id_s']==$els[0]){
				if ($els[1]==0) echo '<div style="margin-left:-20px;" >№ производственной партии <b>'.(date("dmy")).'</b><br></div>'; 
				else echo '<div style="margin-left:-20px;" >№ производственной партии <b>'.$els[1].'</b><br></div>'; 
			}
		}
		fclose($file);
        print '
