<?php 
	...
	if (isset($_GET['task'])){
		if($_GET['task']=='getProduction'){
			$get_production_query="SELECT id, format_name, glass_color, units_number, `type`, boxing FROM productionutf8 WHERE isDeleted='0' ORDER BY id DESC";
			$res=mysql_query($get_production_query);
			echo '{"production":{';
			$delimiter="";
			if (mysql_num_rows($res)>0){
				while($row=mysql_fetch_assoc($res)){
					echo $delimiter.' "'.$row['id'].'":{"id":"'.$row['id'].'", "color":"'.$row['glass_color'].'", "boxing":"'.$row['boxing'].'", "count":"'.$row['units_number'].'", "formatName":"'.$row['format_name'].'", "type":"'.$row['type'].'"}';
					$delimiter=",";
				}
